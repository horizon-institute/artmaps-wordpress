		...
		.ready(
				function() {
					...
					jQuery("#tabs li a").click(function(event) {
						...
						if (jQuery(this).hasClass('inactive')) { // this is
																	// the start
																	// of our
																	// condition
					...
					jQuery("#my-location").click(function(event) {
						jQuery("body").removeClass("menu-expanded"); // Close
																		// responsive
																		// menu
						jQuery(".ui-dialog-content:visible").dialog("close"); // Close
																				// dialogs
						window.main_map.centerOnMyLocation();
						event.preventDefault();
					});
					...
					var config = {
						"map" : {
							"center" : new google.maps.LatLng(51.507854,
									-0.099462), /* Tate Britain */
							"mapTypeId" : google.maps.MapTypeId.ROADMAP
						}
					};
					window.main_map = new ArtMaps.Map.MapObject(
							jQuery("#artmaps-map-themap"), config);
					var autocomplete = jQuery("#artmaps-map-autocomplete");
					main_map
							.bindAutocomplete(new google.maps.places.Autocomplete(
									autocomplete.get(0)));

					(function() {
						var con = jQuery("#artmaps-map-object-container")
								.children().first();
						jQuery("#artmaps-map-object-container").detach();
						ArtMaps.Map.UI.formatMetadata = function(object,
								metadata) {
							var c = con.clone();
							c.find("a").attr(
									"href",
									ArtMapsConfig.SiteUrl + "/object/"
											+ object.ID);
					...
					function show_page(name, title) {
						...
						this.dialog.dialog({
							...
							"dialogClass" : "artwork-results " + name
									+ "-sidebar",
							"resizable" : false,
							"closeText" : "",
							"draggable" : false,
							"open" : function() {
								jQuery(".ui-dialog:visible")
										.removeAttr('style');
								jQuery("time").timeago();
					...
					jQuery("#how-it-works, #more-info").click(function(event) {
						...
						jQuery(".ui-dialog-content:visible").dialog("close"); // Close
																				...
																				// dialogs
						jQuery.fancybox.close(); // Close artwork page
						jQuery("#about-sidebar").html("").load("about"); // Load
																			// content
					...
					jQuery(document).on("click",
							"#account-button, .log-in-trigger",
							function(event) {
								show_page("account", "Account");
								jQuery("#user_login").focus(); // Place cursor
																// in email
																// field
								event.preventDefault();
							});
					...
					case "about":
						jQuery("#how-it-works, #more-info").click();
					case "activity":
					...
					function show_object_modal(object_id) {
						...
						jQuery
								.fancybox({
									href : ArtMapsConfig.SiteUrl + "/object/"
											+ object_id,
									type : 'ajax',
									openEffect : 'none',
									closeEffect : 'none',
									...
									beforeShow : function() {
										...
										jQuery("time").timeago();
										set_page_title(jQuery(
												"#artmaps-object-metadata h1")
												.text());
									},
					...
					jQuery(document).on("click", "#full-image",
							function(event) {
					...
					jQuery(document).on("click", "#menu-toggle",
							function(event) {
								jQuery("body").toggleClass("menu-expanded");
							});

					jQuery(document).on("click",
							"#search-toggle, #close-search", function(event) {
					...
					jQuery(document).on(
							...
							".artwork-link",
							function(event) {
								var object_id = jQuery(this).attr(
										'data-object-id');
								show_object_modal(object_id);
								event.preventDefault();
							});

					jQuery(document).on("click", "#comment-loc-indicator",
							function(event) {
								event.preventDefault();
							});
					...
					search_mode_select
							.change(function() {
								var id = jQuery(this).find("option:selected")
										.attr("id");

								switch (id) {
								case "search-mode-places":
									jQuery(
											'#location-search-form, #search-label-places')
											.show();
									jQuery(
											'#keyword-search-form, #search-label-art')
											.hide();
									jQuery('#location-search-form .query-field')
											.focus();
									break;
								case "search-mode-artworks":
									...
									jQuery(
											'#keyword-search-form, #search-label-art')
											.show();
									jQuery('#keyword-search-form .query-field')
											.focus();
									break;
								}
							});
					...
					displayArtworks = function(data, currentPage) {
						var art_list = jQuery(document.createElement("ul"));
						art_list
								.addClass("artmaps-map-object-list-container-page-body");
						artworkResults.empty().append(art_list);
						var totalResults = data.hits.hits.length;
						if (totalResults == 0)
							return;

						var totalPages = Math.ceil(data.hits.total
								/ totalResults);

						var nav = jQuery(document.createElement("div"));
						nav
								.addClass("artmaps-map-object-list-container-page-nav");
						nav
								.append('<span class="artmaps-map-object-list-container-page-current">Page '
										+ currentPage
										+ " of "
										+ totalPages
										+ '</span>');
						...
						if (currentPage > 1) {
							nav
									.append(jQuery(document.createElement("a"))
											.addClass(
													"artmaps-map-object-list-container-page-previous artmaps-button")
											.text("Prev")
											.one(
													"click",
													function() {
						...
						if (currentPage < totalPages) {
							nav
									.append(jQuery(document.createElement("a"))
											.addClass(
													"artmaps-map-object-list-container-page-next artmaps-button")
											.text("Next")
											.one(
													"click",
													function() {
						...
						jQuery
								.each(
										data.hits.hits,
										function(i, e) {
											var na = jQuery(document
													.createElement("a"));
											na.addClass("artwork-link").attr(
													"href", "#");
											var nc = jQuery(document
													.createElement("li"));
											nc
											...
											try {
												if (e.source.masterImages.length > 0) {
													var oi = e.source.masterImages[0];
													var ni = jQuery(document
															.createElement("img"));
													ni
															.attr(
																	"src",
											...
											na
													.append(jQuery("<h2>"
															...
															+ "</h2>"));
											na
													.append(jQuery('<em>by <span class="artmaps-map-object-container-artist">'
															+ e.source.allArtists
															+ '</span></em>'));
											art_list.append(nc);
										});

						jQuery(".ui-dialog-content").not(
								"#artmaps-search-results-artworks").dialog(
								"close");
						jQuery("#artmaps-search-results-artworks")
								.dialog(
										{
											"width" : 260,
											...
											"close" : function() {
												jQuery(
														"#keyword-search-form input.query-field")
														.val("");
												jQuery.fancybox.close();
											},
											"title" : ''
					...
					searchForm.submit(function() {
						_gaq.push([ "_trackEvent", "Search", "Keyword",
								searchInput.val() ]);
						jQuery('#welcome').fadeOut(300);
						jQuery.ajax(ArtMapsConfig.AjaxUrl, {
							"type" : "post",
					...
					function fixHeightOnIOS7() {
						var fixedHeight = Math.min(jQuery(window).height(), // This
																			// is
																			// smaller
																			// on
																			// Desktop
						window.innerHeight || Infinity // This is smaller on
														// iOS7
						);
